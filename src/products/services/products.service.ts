...
export class ProductsService {
  ...
  async findMany(
    ...
    const catx = category ? { category: { $regex: category, $options: 'i' } } : {};
  
    console.log('Constructed filters:', { ...rgex, ...catx });
  
    const count = await this.productModel.countDocuments({ ...rgex, ...catx });
    const products = await this.productModel
      .find({ ...rgex, ...catx })
      .sort({ createdAt: -1 })
      .limit(pageSize)
  ...
  async update(
    ...
    const { name, price, imagesSlides , isTopRelated , description, image, brand, category, tags, countInStock } = attrs;
